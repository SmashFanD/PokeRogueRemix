...
//This should increase all positives stats at once instead of one by one
//Same for negatives
export class StatStageChangePhase extends PokemonPhase {
  phaseName = "StatStageChangePhase";
  stats;
  selfTarget;
  stages;
  showMessage;
  ...
  start() {
    //Check for positive then negative instead
    ...
    if (pokemon.isPlayer()) {
      ...
      if (this.comingFromStickyWeb) {
        ...
        const stickyTagID = globalScene.arena.findTagsOnSide(
          (t) => t.tagType === ArenaTagType.STICKY_WEB,
          ArenaTagSide.PLAYER,
        )[0].sourceId;
  ...
  getStatStageChangeMessages(stats, stages, relStages) {
    const messages = [];
