...
export class MovePhase extends PokemonPhase {
  ...
  // TODO: Do we need getters but only setters?
  set pokemon(pokemon) {
  ...
  start() {
    ...
    const encoreTag = user.getTag(BattlerTagType.ENCORE);
  ...
  getPpIncreaseFromPressure(targets) {
  ...
  useMove(charging = false) {
    ...
    // @ts-expect-error - useMode is and shouldn't normally be assigned to
  ...
  thirdFailureCheck() {
    ...
    if (!failed) {
      ...
      defendingSidePlayField.forEach((pokemon) => {
  ...
  executeMove() {
    ...
    const dancerModes = [MoveUseMode.INDIRECT, MoveUseMode.REFLECTED];
   ...
   */
  showFailedText(failedText = i18next.t("battle:attackFailed")) {
   ...
   */
  failMove(failedDueToTerrain = false) {
    const move = this.move.getMove();
    const targets = this.getActiveTargetPokemon();
    const pokemon = this.pokemon;
   ...
   */
  triggerStatus(effect, cancel = true) {
    const pokemon = this.pokemon;
