...
export class MoveEffectPhase extends PokemonPhase {
   ...
   */
  constructor(battlerIndex, targets, move, useMode) {
    super(battlerIndex);
    this.move = move;
  ...
  start() {
    ...
    /**
     * Does an effect from this move other effects on this turn?
     * e.g. Charging moves (Fly, etc.) on their first turn of use.
     */
    const overridden = new BooleanHolder(false);

    // Apply effects to a move effect.
   ...
   */
  conductHitChecks(user, fieldMove) {
   ...
   * @see {@linkcode hitCheck}
   */
  checkBypassAccAndInvuln(target) {
    const user = this.getUserPokemon();
   ...
   */
  checkBypassSemiInvuln(semiInvulnerableTag) {
   ...
   */
  applyMoveEffects(target, effectiveness, firstTarget) {
    const user = this.getUserPokemon();
   ...
   */
  applyMove(user, target, effectiveness) {
   ...
   * @returns A {@linkcode MoveDamageTuple} containing the results of damage application.
   */
  applyMoveDamage(user, target, effectiveness) {
   ...
   */
  applyHeldItemFlinchCheck(user, target, dealsDamage) {
 ...
 */
function isMoveReflectableBy(move, target, useMode) {
  return (
    !isReflected(useMode)
