...
export class SummonPhase extends PartyMemberPokemonPhase {
  phaseName = "SummonPhase";
  loaded;

  constructor(fieldIndex, player = true, loaded = false) {
  ...
  preSummon() {
    ...
    if (!partyMember.isAllowedInBattle() || (this.player && !this.getParty().some(p => p.id === partyMember.id))) {
      console.warn(
        "The Pokemon about to be sent out is fainted or illegal. Attempting to resolve...",
      );
    ...
    if (this.player) {
      ...
      const trainerName = globalScene.currentBattle.trainer?.getName(TrainerSlot.TRAINER);
  ...
  /**
   * Remove pokeballs throw, they are not in balls
   */
  summon() {
  ...
  onEnd() {
    ...
    if (
      !this.loaded
      || [BattleType.TRAINER].includes(globalScene.currentBattle.battleType)
      || globalScene.currentBattle.waveIndex % 5 === 1
    ) {
